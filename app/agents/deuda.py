...
_DEUDAS = (
    dict(id="D-001", gravedad=FALSEA, capa="C1/C3", dueño="Javo",
         ...
         medicion_008="GM-Ω-ICPI-008 (2026-09-03) la midió y encontró TRES "
                      "cosas. **(1) YA ESTABA DECIDIDA**: `ADR-036`, ratificado "
                      "el 2026-07-15, declara las 25 como universo operacional "
                      "de v1, verifica que todas existen en el PDOT y congela el "
                      "motor — ampliar a 66 sería evolución, no corrección. "
                      "**(2) EL CRITERIO YA ESTÁ DECLARADO** (2026-09-03): Javo lo "
                      "fijó —«las 25 fueron tomadas por contener el monto "
                      "económico más amplio, para fines sólo de tesis»—. La "
                      "muestra representa el GASTO, no el mandato: el sistema "
                      "institucional queda al 15 % de cobertura, y ahí viven "
                      "gobernanza, transparencia y participación. **(3) LA OBLIGACIÓN DEL ADR-036 §1 NO SE "
                      "CUMPLIÓ**: manda declarar el alcance en toda publicación "
                      "de d01/d03 y aparece en **0 superficies visibles** — sólo "
                      "en un comentario de código. Además, la aritmética del "
                      "complemento no cuadra (66−25=41 pero el catálogo de "
                      "exclusiones tiene 50).",
    ...
    dict(id="D-015", gravedad=FALSEA, capa="C6", dueño="Javo",
         que="**Cuatro cifras circulan como `SITA 2025`.** `governance/"
             "BOOT.md` publicaba **0,4448**, valor **no respaldado por los "
             "artefactos examinados** —⚠️ lo que no demuestra que carezca de "
             "origen: pudo ser una corrida no conservada, una versión "
             "intermedia o una transcripción, y establecerlo pertenece a esta "
             "deuda—. `PCD-D07` afirma **0,4646**. Y las corridas selladas "
             "traen **dos** valores para el mismo año y los mismos doce "
             "meses: `-0001` **0,9719** y `-0002` **0,4630**, ambas "
             "`COMPLETED`, mismo `vara_sha`.\n\n"
             "★ LA DIVERGENCIA 0,9719 ↔ 0,4630 QUEDA RESUELTA (forense "
             "2026-09-09). No fue metodología: fue **universo de entrada**.\n"
             "  `RUN-D07-2026-08-18-0001`  21 CD · 138 filas · **0 en cero** "
             "→ 0,9719\n"
             "  `RUN-D07-2026-08-18-0002`  24 CD · 240 filas · 124 en cero "
             "→ 0,4630\n"
             "Los tres CD que `-0001` no incluye —`CD-01`, `CD-07`, `CD-10`— "
             "son **exactamente los tres no publicados en el portal**, con "
             "12/12 filas en cero y la observación literal «Sin información: "
             "el conjunto no está publicado en el portal».\n\n"
             "⚠️ `-0001` **excluyó del universo las ausencias**: midió sólo lo "
             "que existía. No es una medición alternativa de 2025 — es una "
             "corrida **parcial mal encuadrada**, y por eso arroja un "
             "municipio casi perfecto. Es literalmente lo que `ADR-046` "
             "advierte: así «el sistema premiaría la opacidad». `-0003` "
             "(2026) usa los 24 CD, coherente con `-0002` y no con `-0001`.\n\n"
             "RESIDUO ABIERTO: `-0002` da 0,4630 y `PCD-D07` afirma 0,4646. "
             "Y hay una pista sobre la diferencia — el `PCD` dice **2** "
             "conjuntos sin publicar y la corrida muestra **3**—, lo que "
             "sugiere una tercera corrida con otro universo, no conservada en "
             "`data/d07/corridas/`.",
         ataque="test_una_corrida_no_excluye_del_universo_lo_no_publicado",
         no_es="un error de cálculo del motor, y **tampoco autoriza a elegir "
               "una de las cuatro**. La fórmula `SITA` está verificada contra "
               "el Instructivo DPE 2024 y `BOOT` ordena ⛔ **NO tocar SITA**: "
               "esta deuda es de **procedencia del dato publicado**, no de "
               "metodología. Y la pregunta correcta no era «¿cuál SITA 2025 "
               "es la buena?» sino **«¿qué diferencia de estado de entrada "
               "produjo dos resultados bajo la misma vara?»** — la primera "
               "invita a escoger, la segunda obliga a reconstruir. ⚠️ De ahí "
               "la regla que esta deuda deja: **mismo `vara_sha` no significa "
               "mismo estado de entrada**; acredita la regla aplicada, nunca "
               "el universo sobre el que se aplicó. Hasta cerrar el residuo, "
               "`SITA 2025` es **NO DETERMINABLE**"),
    ...
    dict(id="D-013", gravedad=FALSEA, capa="C1", dueño="Javo",
         que="**Tres reglas discrepantes definen el mismo factor `C_i`, y una "
             ...
             "`H02!Ci_Determinista` y la matriz normativa `H01` Sección L "
             "definen ambos las deducciones y **no coinciden**: `INF-03` deduce "
             "`0,05` en la matriz y `0,20` en el glosario (×4); `INF-04` **FIJA** "
             "`Ci=0,50` en la matriz y **resta** `0,50` en el glosario (son "
             "operaciones distintas); el piso es `MÁX(0,50; …)` en la matriz y "
             "`MAX(…, 0)` en el glosario —con el piso alto, `C_i` **nunca puede "
             "anular una meta**—. Y la Sección L declara que el motor «abandona "
             "la valoración heurística» mientras la M declara que "
             "`Ci_Manual_2025` es el fallback vigente. Lo encontró GM-Ω-011-C2.",
         ataque="test_las_reglas_discrepantes_de_Ci_quedan_registradas",
         no_es="un error activo del ICPI publicado: hoy **ninguna de las tres "
               "reglas se ejecuta**, porque las cuatro columnas de infracción "
               "están en cero para las 25 metas. Es una divergencia LATENTE, y "
               "eso la hace más fácil de corregir, no menos urgente — se "
               "activaría la primera vez que se registre una infracción, que es "
               "justo cuando el motor tiene que estar bien (DOC-025). Y NO se "
               "arregla eligiendo la que produzca mejor número: cuál rige lo "
               "determina `011-C3` con procedencia documental"),
    ...
    dict(id="D-011", gravedad=FALSEA, capa="C4/C6", dueño="Javo",
         que="La capa de publicación describe el ICPI con un vocabulario que el "
             ...
             "DEFINICIÓN**: `data/gm_snapshot.json` publica el ICPI como «Índice "
             "Compuesto de Progreso Institucional» y anota que «mide velocidad "
             "de ejecución», cuando el constructo reconstruido en "
             "GM-Ω-ICPI-001 desde la tesis es «Índice de Congruencia "
             "Programática e Intersistémica» y NO mide velocidad: mide si la "
             "cadena programa→norma→verificación→ejecución→tiempo→trazabilidad "
             "se sostiene entera. Un nombre y una definición que el motor no "
             "respalda, viajando en el artefacto que consume la UI. "
             "**(2) LA CLASIFICACIÓN**: `H12!B34` sólo "
             "emite categoría con doce meses de corte (`H07!B22>=12`) y el corte "
             "vigente es abril, así que devuelve «Corte parcial - lectura "
             "preliminar (no comparable con umbral anual)». La capa API la "
             "propaga literal en `H73!ICPI_CLASIFICACION`, el conector la lee en "
             "`snapshot['icpi']['clasificacion']` y **cinco superficies** la "
             "publican: `p6_pulso`, `p7_brecha`, `p_command_center`, `p_concejo` "
             "y `p_ejecutivo`. Son dos defectos encadenados: donde debía haber "
             "una categoría de gobernanza hay una frase que no lo es, y esa "
             "frase está escrita en lenguaje interno —«umbral anual», «corte "
             "parcial»— cruzando la frontera que el Bloomberg Firewall existe "
             "para sostener. Las dos las encontró GM-Ω-ICPI-007 al montar el "
             "laboratorio de sensibilidad, no una revisión de la UI: es el "
             "patrón «derivado narrativo desacoplado de la fuente canónica» "
             "—el del 48,33 %— aplicado al vocabulario de la cifra madre.",
         ataque="test_el_vocabulario_publicado_del_ICPI_corresponde_al_motor",
         no_es="un error del motor: `B34` hace lo correcto al negarse a emitir "
               "una categoría anual con cuatro meses de datos —esa prudencia es "
               "exactamente la doctrina de QUIRA—. El defecto está en que el "
