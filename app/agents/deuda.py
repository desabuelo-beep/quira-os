...
_DEUDAS = (
    ...
    dict(id="D-015", gravedad=FALSEA, capa="C6", dueño="Javo",
         que="**El valor vigente de `SITA 2025` no tiene corrida sellada que "
             ...
             "  `0,9719` · `RUN-D07-2026-08-18-0001` · **21 CD** · 138 filas · "
             "**0 en cero** — excluyó del universo `CD-01`, `CD-07` y `CD-10`, "
             "los tres que el portal no publica (12/12 filas en cero cada uno, "
             "«Sin información: el conjunto no está publicado en el portal»). "
             "Corrida **parcial mal encuadrada**\n"
             "  `0,4630` · `RUN-D07-2026-08-18-0002` · 24 CD · 240 filas · 124 "
             "en cero — universo completo, **pre-acreditación**\n"
             "  `0,4646` · citado por `PCD-D07` — pre-acreditación\n"
             "  `0,4448` · **VIGENTE** — `76ca5de` (20-ago), al acreditar "
             "`enlaces` e `inventario`. Sólo `CI` se movió (0,4792→0,4000), y "
             "el commit lo explica: «la medición anterior contaba como bueno "
             "lo que nunca se había mirado». No se ha movido desde entonces\n\n"
             "⚠️ LO QUE FALTA, y son dos cosas distintas:\n"
             "  1 · las corridas de `data/d07/corridas/` son del **18-ago** y "
             "nunca se regeneraron tras la acreditación del **20-ago**: el "
             "valor vigente **no tiene corrida conservada que lo reproduzca**\n"
             "  2 · `PCD-D07` sigue citando `0,4646`, superado por `76ca5de`. "
             "⛔ **NO se corrige el expediente para que cuadre**: primero la "
             "genealogía, y está bajo preservación (`REARQ-001`)\n\n"
             "Y una tercera, de forma: `59c90bf` (2-sep) retiró de `BOOT` el "
             "paréntesis «(era 0,4646: se contaba bueno lo no mirado)». La "
             "cifra quedó correcta y **sin su genealogía** — que es "
             "exactamente como una lectura posterior la tomó por infundada y "
             "estuvo a punto de retirarla.",
         ataque="test_una_corrida_no_excluye_del_universo_lo_no_publicado",
         no_es="una cifra equivocada en `BOOT` —`0,4448` **es** la correcta— "
               "ni un error de cálculo. La fórmula `SITA` está verificada "
               "contra el Instructivo DPE 2024 y `BOOT` ordena ⛔ **NO tocar "
               "SITA**. Es una deuda de **procedencia**: falta el artefacto "
               "sellado que reproduzca el valor vigente, y sobra un expediente "
               "que cita el anterior. ⚠️ Deja dos reglas: **mismo `vara_sha` "
               "no significa mismo estado de entrada** —acredita la regla "
               "aplicada, nunca el universo sobre el que se aplicó— y **el "
               "universo de una medición son los conjuntos EXIGIBLES, no los "
               "ENCONTRADOS**, porque medir sólo lo publicado premia la "
               "opacidad (`ADR-046`)"),
