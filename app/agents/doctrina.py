...
_DOCTRINA = (
    ...
    dict(id="DOC-036", custodia=GATE,
         regla="**Protocolo forense de búsqueda.** ⛔ PASO 0, ANTES DE TODO: "
               "**consultar `governance/QUIRA_MASTER_INDEX.md`** — la tabla "
               "de autoridad que responde «¿dónde vive la verdad de X?» y "
               "rutea al RECTOR. Existe desde 2026-06-21 para eso. Y "
               "`CodeGraph` · `graphify-out/` para preguntas estructurales, "
               "antes que `grep`.\n\n"
               "⛔ **PASO 0-bis, y el Index NO lo sustituye:** antes de "
               "declarar cualquier hallazgo ARQUITECTÓNICO, barrer el "
               "INVENTARIO COMPLETO de `docs/` y `governance/` por título y "
               "propósito —275 documentos, minutos de lectura—. El `Index` "
               "rutea por *tipo de verdad*, no por *documento*: consultarlo es "
               "necesario y **no suficiente**. `Q-M2` lo pagó reconstruyendo "
               "en tres días lo que `ADR-033` y `ADR-042` ya decidían.\n\n"
               "Sólo entonces, si ni el índice ni el inventario responden: 1 "
               "declarar universo · 2 identificar "
               ...
               "existe»**.\n\n"
               "⚠️ Y NO significa «buscarlo todo antes de cada acción» —eso "
               "también sería ineficiente—: significa **ORIENTARSE antes de "
               "buscar**. La secuencia es `TAREA → BOOT → MASTER_INDEX → "
               "RECTORES/SSoT → GRAFO → DOCUMENTACIÓN ESPECÍFICA → "
               "CÓDIGO/DATOS/HISTORIA → BARRIDO EXHAUSTIVO SÓLO DEL UNIVERSO "
               "QUE SIGA ABIERTO → CONCLUSIÓN`. Primero orientación, después "
               "lectura dirigida, y arqueología exhaustiva **sólo donde quede "
               "incertidumbre**.\n\n"
               "⛔ El coste en tokens no es criterio para decidir qué "
               "evidencia mirar: la economía de tokens no puede convertirse "
               "en economía de evidencia. **No se diseña sobre el recuerdo de "
               "QUIRA: se diseña sobre su evidencia completa.**",
         ...
         por_que_ahi="`DOC-035` fija el principio; ésta fija el PROCEDIMIENTO, "
                     ...
                     "conclusiones**.\n\n"
                     "⚠️ Y el motivo del PASO 0, medido: en esta sesión se "
                     "reconstruyeron con `grep` cosas ya escritas y ruteadas. "
                     "El `MASTER_INDEX` —que dice de sí mismo «esto mata para "
                     "siempre el *creo que ya existía*»— no se abrió ni una "
                     "vez, pese a estar citado en `BOOT` línea 12. Quedaron "
                     "sin leer 112 KB de rectores indexados, entre ellos "
                     "`METODOLOGIA_D07_CUMPLIMIENTO_LOTAIP` —el rector del "
                     "`SITA`, mientras se investigaba el `SITA`—. Y se estuvo "
                     "a punto de proponer un «contrato de evidencia "
                     "interdominio» que **ya existe**: `MISMA_FUENTE_QUE` en "
                     "Neo4j, siete `scripts/cypher/`, con el principio ya "
                     "escrito —«la cédula se extrae una vez, no dos»—. "
                     "Inventar lo que ya está viola además la `Regla de Oro "
                     "7`. **La búsqueda exhaustiva empieza por el índice, no "
                     "por el barrido**: barrer sin consultar el índice es "
                     "caro, lento y produce hallazgos que ya eran canon"),
    dict(id="DOC-035", custodia=GATE,
         regla="⚠️ DERIVADA de `ADR-042 §6-quinquies` —«hallar un término "
               "prueba presencia; no hallarlo NO prueba ausencia»—, que ya "
               "gobernaba este principio para la búsqueda en los documentos "
               "del SUJETO OBSERVADO. Esta doctrina lo extiende al OBSERVADOR "
               "buscando en su propio corpus. Mismo principio, otro plano; no "
               "es un hallazgo nuevo y no debe citarse como tal.\n\n"
               "El resultado de una búsqueda sólo autoriza una conclusión de "
