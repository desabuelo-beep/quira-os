...
📚 **ADR-038 BRN v2 RATIF:** nodo = REGLA, no artículo. 4 niveles **Corpus→CNO(Derecho)→RO(lógica)→SAT**. BRN = CAPA (no cajón) que CONSOLIDA (no interpreta). Canon legal ya en Supabase+`Normativa_Word` (43 docs · sin Lexis). **BRN v2.0 CONGELADA**: d02+d03 VIGENTES·ROAdapter·suite 11/11✓. Sig: d04 sin tocar infra.
