...
- **📊 AUDITORÍA DE MIGRACIÓN — Sprint D.1.5 (2026-06-16 · `docs/architecture/AUDITORIA_MIGRACION_D1.5.md`):** KPI brutal → **migración ≈ 8%** (solo d06 lee el motor real). El **L1 tiene 8 métricas hardcodeadas, 3 CONTRADICEN el motor** (d05 Holding 68.7 vs ~17 · d08 Participación 27.98 vs 48.33 · d12 Género 12.83 vs 2.83 = "el enemigo" del colega, en vivo). ~14 pantallas activas leen `load_all`/demo (p3·p4·p8·p9·p10·p11·p12·p13·p14·p16·p17·p18·p19). Patrón `GM→snapshot→pantalla` **PROBADO** (d06) — falta replicarlo a d01-d12.
    ▶ **SIGUIENTE = Sprint D.2 replicación** (cosechar cantera → forma de cajón · eliminar demo/hardcode · frontera de lenguaje · builder por cajón · prioridad: los 3 hardcodes que contradicen el motor d05·d08·d12). ⏳ Javo: **revocar el token GitHub** (yo ya saqué el token de `.git/config` + GCM configurado + verificado sin commitear).
