...
- **✅ CIRUGÍA GOLD MASTER D.2A — COMPLETA Y VERIFICADA INEXPUGNABLE (2026-06-15 · sobre COPIA · Excel nativo + dump):**
    ...
    **cablear ANTES de persistir** (3A cablear→deploy→auditoría visual · 3B Supabase DESPUÉS, no antes).
    ...
    ▶ SIGUIENTE: cablear `p6_pulso` + `p7_brecha` **YA con lenguaje de gobernanza** (`load_all` demo→local · 4 congruencias→"Pendiente C3" · 6 vectores→`_raw_h73` con etiquetas públicas · purgar hardcodes "$40/hab") · re-correr `quira-language-guard` · **auditoría visual (abrir d06 · ¿pantalla==27.46%? = EL HITO)** · **3B persistir Supabase = OBLIGATORIO** (mata el 17.45% stale). Matriz-spec: `docs/architecture/d06_MAPPING_MATRIX.md`.
