...
📚 **ADR-038 BRN v2 RATIF:** nodo = REGLA, no artículo. 4 niveles **Corpus→CNO(Derecho)→RO(lógica)→SAT**. BRN = CAPA (no cajón) que CONSOLIDA (no interpreta). Canon legal ya en Supabase+`Normativa_Word` (43 docs · sin Lexis). **Ciclo+molde CNO/RO✓** · Sig: piloto CNO-IV-001 · catálogo CNO · plano maestro.
