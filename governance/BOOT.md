...
- **✅ CIRUGÍA GOLD MASTER D.2A — COMPLETA Y VERIFICADA INEXPUGNABLE (2026-06-15 · sobre COPIA · Excel nativo + dump):**
    ...
    **cablear ANTES de persistir** (3A cablear→deploy→auditoría visual · 3B Supabase DESPUÉS, no antes).
    ...
    ▶ SIGUIENTE: cablear `p6_pulso` (`load_all` demo→local · 4 congruencias→placeholder C3 · purgar hardcodes) + `p7_brecha` (6 vectores→`_raw_h73`) · **auditoría visual (abrir d06 · ¿pantalla==27.46%? = EL HITO)** · **3B persistir Supabase = ahora OBLIGATORIO** (mata el 17.45% stale). Matriz-spec: `docs/architecture/d06_MAPPING_MATRIX.md`.
