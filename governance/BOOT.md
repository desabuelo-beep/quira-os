...
- **🔧 SPRINT D.2 ARRANCÓ — los 3 hardcodes del L1 que MENTÍAN, cableados al motor (2026-06-17 · commit `f292475` · EN LA NUBE):** L1 Centro de Mando v2 + banda mini del drill-down (`env_gov`) leen el snapshot real → **Holding 68.7→12.4%** (consolidado ponderado h90) · **Participación 27.98→48.33%** (vectores.igp) · **Género 12.83→2.83%** (vectores.psg). + ganchos veraces sin cifras-hardcode (eliminados "82%"/"40%"/"17.2pp·$2.1M") · banda drill-down ICPI motor-primero (mata el 17.45% stale de Supabase) sin "umbral 65%" · frontera de lenguaje verificada (guard). **El "enemigo" de las verdades simultáneas, muerto en el L1.**
    ▶ **d05 COMPLETO (2026-06-17 · `ea3d6df`):** detalle `p2_holding` abre con el "Eje de Consistencia" — 12.4% desglosado en las 4 entidades del motor (h90), cierra el círculo causal; radar SERCOP real (Supabase) intacto debajo. Mata el "EP Aseo líder" falso (manda Patronato 19.56). **d05 = 100% migrado MEDIDO** (tarjeta h90 + detalle h90 + SERCOP Supabase · 0 demo · 0 contradicción).
    ▶ **d08 CONVERGIDO (2026-06-17 · `6d92649`):** `p16_confianza` headline 27.98 huérfano → 48.33 motor + **purga firewall CRÍTICA** ("IGP"/"H73"/"Gold Master v4.1" estaban VISIBLES en producción) + narrativa veraz (sin umbral falso 40%) + 6 mecanismos reales preservados. ⚠️ Honesto: el motor expone SOLO el compuesto (48.33), NO un desglose; los mecanismos son hechos curados reales (no snapshot-backed). d08 = headline+narrativa+firewall ✅ (NO "100% motor" como d05).
    ▶ **🔥 SPRINT D.2A — FIREWALL DEBT MEDIDO (2026-06-17 · `19b68b3` · `docs/architecture/FIREWALL_DEBT_REPORT.md`):** escáner AST `scripts/dev/firewall_audit.py` → **190 fugas VISIBLES en 25 pantallas** (2º enemigo: metodología expuesta, no solo `demo_data`). Validación: p_ejecutivo+p16 = **0 ALTO** (el patrón de convergencia limpia firewall). ~150 público · ~22 interno-ops legítimo (`env_ops`/`p_carga`) · ~23 verificar (¿`is_tecnico`?). Baseline `firewall_debt.json` (re-correr → deuda a 0 MEDIDA).
    ▶ **🏁 d12 DOUBLE-KILL + OLA 1 COMPLETA (2026-06-17 · `fea4d28`):** `p19_genero` headline PSG 12.83→2.83 motor **Y** firewall 22→0 en un frente. **Deuda repo MEDIDA: 190 → 168** (-22, exacto a la proyección de la mesa). **Los 3 hardcodes críticos (d05·d08·d12) NEUTRALIZADOS.** Honesto: el motor expone solo el compuesto (como d08); IGM/programas = hechos curados. Vista Ejecutivo = QTMP canónico (lente Protección Social más amplio).
    ▶ **SIGUIENTE:** barrer públicas por deuda de firewall con el escáner (`p_cadena_institucional` 14 · `p16_gobernanza` 13 · `p14_eficiencia` 11 · `p12_cadena` 10 · `p13_simulador` 9 · `p11_ods` 9 …) → deuda a 0 MEDIDA. Verificar gating de `m2_alertas`/`p_sentinel_hub` (¿`is_tecnico`?). NO purgar interno-ops (`env_ops`/`p_carga` = consolas Dylus legítimas). + V1 `p_command_center` DORMIDO (fallback).
- **📊 AUDITORÍA DE MIGRACIÓN — Sprint D.1.5 (2026-06-16 · `docs/architecture/AUDITORIA_MIGRACION_D1.5.md`):** KPI brutal → **migración ≈ 8%** (solo d06 lee el motor real). El **L1 tiene 8 métricas hardcodeadas, 3 CONTRADICEN el motor** (d05 Holding 68.7 vs ~17 · d08 Participación 27.98 vs 48.33 · d12 Género 12.83 vs 2.83 = "el enemigo" del colega, en vivo). ~14 pantallas activas leen `load_all`/demo (p3·p4·p8·p9·p10·p11·p12·p13·p14·p16·p17·p18·p19). Patrón `GM→snapshot→pantalla` **PROBADO** (d06) — falta replicarlo a d01-d12.
    ▶ **SIGUIENTE = Sprint D.2 replicación** (cosechar cantera → forma de cajón · eliminar demo/hardcode · frontera de lenguaje · builder por cajón · prioridad: los 3 hardcodes que contradicen el motor d05·d08·d12). ✅ **Seguridad RESUELTA:** token expuesto ya no existe (lista classic vacía) · `.git/config` limpio · GCM OAuth seguro · todo pusheado a la nube (d06 vivo en `quiraholding.streamlit.app`). 🏁 Cierre 2026-06-16 22h19 ECU.
