...
Campos v2.0 (Schema Star — ADR-021):
  document_class   : 'NORMA' | 'METODOLOGIA' | 'INSTRUMENTO_TERRITORIAL' | 'EVIDENCIA_OBSERVACIONAL'
  authority_level  : int 10-100 (peso normativo)
  source_entity    : entidad emisora canónica
  canton_id        : None (normas nacionales) | 'MCR' (Montecristi)
...
MANIFEST: list[dict] = [
    ...
    },
    {
        ...
        "dominios":         ["Dom02","Dom04","Dom09"],
        "vigente":          True,
        # NORMA/60 — vinculante por COPLAFIP Art.12 (no METODOLOGIA/50)
        "document_class":   "NORMA",
        "authority_level":  60,
        "source_entity":    "SNP",
        "canton_id":        None,
...
def validate_entry(e: dict) -> None:
    """Valida que una entrada del MANIFEST tenga todos los campos obligatorios v2.0."""
    required = ["archivo", "sigla", "nombre", "jerarquia", "milestone", "tipo",
                "dominios", "vigente", "document_class", "authority_level", "source_entity"]
    for field in required:
        if field not in e:
            raise ValueError(f"Entrada '{e.get('sigla','??')}' falta campo obligatorio: {field}")
    if e["authority_level"] < 10 or e["authority_level"] > 100:
        raise ValueError(f"authority_level debe estar entre 10-100: {e['sigla']}")
    valid_classes = {"NORMA","METODOLOGIA","INSTRUMENTO_TERRITORIAL","EVIDENCIA_OBSERVACIONAL"}
    if e["document_class"] not in valid_classes:
        raise ValueError(f"document_class inválido en {e['sigla']}: {e['document_class']}")
...
if __name__ == "__main__":
    ...
    for e in MANIFEST:
        try:
            ...
            invalid += 1
    if invalid == 0:
        print(f"\n  Validacion v2.0: todos los campos OK")
    else:
        print(f"\n  Validacion v2.0: {invalid} entradas con errores")
